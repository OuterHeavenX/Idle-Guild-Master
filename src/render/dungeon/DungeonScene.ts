...
export class DungeonScene extends Container {
  ...
  private heroes: HeroActor[]=[]; private enemy=new EnemyActor(); private hud: DungeonHud; private feed=new LootFeed(); private fx=new CombatEffects(); private time=0; private width=1; private height=1;
  ...
  resize(width:number,height:number):void{ this.width=width;this.height=height;this.drawEnvironment(); const y=height*.62; const pts:[[number,number],[number,number],[number,number],[number,number]]=[[width*.19,y],[width*.36,y+35],[width*.24,y+82],[width*.43,y+86]]; this.heroes.forEach((h,i)=>h.position.set(pts[i][0],pts[i][1])); this.enemy.position.set(width*.72,height*.47); this.feed.position.set(10,height*.73); this.feed.resize(Math.min(180,width*.47)); this.hud.update(width,height); }
  update(dt:number):void{ this.time+=dt; this.heroes.forEach(h=>h.update(dt)); this.enemy.update(dt); this.fx.update(dt); this.ambient.y=Math.sin(this.time*.18)*3; this.hud.update(this.width,this.height); }
  private drawEnvironment():void{ this.env.removeChildren().forEach(c=>c.destroy()); this.ambient.removeChildren().forEach(c=>c.destroy()); const w=this.width,h=this.height; const bg=new Graphics().rect(0,0,w,h).fill(0x0b0d16); const arch=new Graphics().roundRect(w*.08,h*.09,w*.84,h*.65,w*.18).fill(0x171826).stroke({color:0x38364a,width:5}); const inner=new Graphics().roundRect(w*.15,h*.16,w*.7,h*.53,w*.14).fill(0x0d101c); const floor=new Graphics().moveTo(0,h*.46).lineTo(w,h*.46).lineTo(w,h).lineTo(0,h).fill(0x17161d); for(let i=0;i<8;i++){floor.moveTo(i*w/7,h*.46).lineTo(w*.5+(i-3.5)*w*.12,h).stroke({color:0x2c2931,width:1,alpha:.65});} for(let j=0;j<5;j++){const yy=h*.49+j*h*.105;floor.moveTo(0,yy).lineTo(w,yy).stroke({color:0x2b2930,width:1,alpha:.55});}
    ...
    this.env.addChild(bg,arch,inner,floor,pillars,chains,torches); const fog=new Graphics(); for(let i=0;i<7;i++)fog.ellipse(w*(.08+i*.15),h*(.42+(i%2)*.08),80,18).fill({color:0x8a89b5,alpha:.025}); const ash=new Graphics(); for(let i=0;i<35;i++)ash.circle(Math.random()*w,Math.random()*h,Math.random()*1.4+.4).fill({color:i%4===0?0xf1aa62:0xb8b2c5,alpha:.18}); this.ambient.addChild(fog,ash); }
  private heroPos(id:string):[number,number]{const h=this.heroes.find(x=>x.heroId===id);return h?[h.x,h.y-25]:[this.width*.3,this.height*.6];}
  private bind():void{
