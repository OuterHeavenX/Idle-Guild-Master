...
export class DungeonHud extends Container {
  ...
  update(width: number, height: number): void {
    ...
    const heroes=this.state.heroes.slice(0,4); const cardW=(width-30)/2; heroes.forEach((h,i)=>{ const col=i%2,row=Math.floor(i/2),x=10+col*(cardW+10),y=height-91+row*39,r=Math.max(0,h.currentHp/h.stats.maxHp); const g=this.heroBars[i]; g.clear().roundRect(x,y,cardW,32,8).fill({color:0x0b0c12,alpha:.82}).stroke({color:0x4d4454,width:1}).roundRect(x+8,y+20,cardW-16,5,3).fill(0x271720).roundRect(x+8,y+20,(cardW-16)*r,5,3).fill(r>.35?0x5fa66d:0xbf4c48); const t=this.heroTexts[i]; t.text=`${h.name} · ${h.jobId.toUpperCase()}`; t.position.set(x+8,y+5); });
  }
}
