...
export class CombatEffects {
  ...
  projectile(x1: number, y1: number, x2: number, y2: number, magic = false): void {
    const node = new Container(); const dot = new Graphics().circle(0, 0, magic ? 5 : 3).fill(magic ? 0x7ac7ff : 0xe8d39a);
    node.addChild(dot); const life = 0.22; this.add(node, x1, y1, life, (x2 - x1) / life, (y2 - y1) / life, 'projectile');
  }
  impact(x: number, y: number, color = 0xffa04d): void { for (let i = 0; i < 7; i++) { const g = new Graphics().circle(0, 0, 2 + (i % 2)).fill(color); const a = (i / 7) * Math.PI * 2; this.add(g, x, y, 0.35, Math.cos(a) * (28 + i * 3), Math.sin(a) * (28 + i * 3), 'burst'); } }
  ...
  update(dt: number): void {
    for (let i = this.effects.length - 1; i >= 0; i--) {
      ...
      if (e.kind !== 'projectile') e.vy += 34 * dt; e.node.alpha = Math.max(0, e.life / e.max);
