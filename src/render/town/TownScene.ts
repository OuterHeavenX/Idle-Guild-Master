...
export class TownScene extends Container{
 ...
 private async build(){
  ...
  const style=new TextStyle({fontFamily:'Georgia',fontSize:28,fill:0xf5e2b7,stroke:{color:0x211810,width:4}});const labels:Array<[string,number,number]>=[['GUILD HALL',230,240],['BLACKSMITH',700,500],['NOTICE BOARD',250,470],['EASTERN GATE',405,128]];for(const[s,x,y]of labels){const text=new Text({text:s,style});text.x=x;text.y=y;this.addChild(text)}
  const npc=(x:number,y:number,body:number,skin=0xc69b78)=>{const g=new Graphics();g.circle(0,0,32).fill(body);g.circle(0,-28,18).fill(skin);g.x=x*1000;g.y=y*1500;this.addChild(g)};npc(.50,.25,0x3f4057);npc(.76,.48,0x6f3b26);npc(.235,.475,0x665c62);npc(.275,.49,0x3e4e45);npc(.31,.47,0x353850);
  this.focusRing.visible=false;this.addChild(this.focusRing);await Assets.load('assets/dungeon/ashen-crypt/heroes/guardian.svg');this.player=Sprite.from('assets/dungeon/ashen-crypt/heroes/guardian.svg');this.player.anchor.set(.5,.82);this.player.width=82;this.player.height=108;this.addChild(this.player);if(debug){this.drawDebug();this.addChild(this.debugLayer)}this.syncPlayer();
 }
