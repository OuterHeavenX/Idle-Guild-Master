...
export class StateManager{
 ...
 apply(save:GameSave):void{this.heroes=save.heroes.map(hero=>new Hero({...hero,currentHp:Math.max(0,Math.min(hero.stats.maxHp,Number.isFinite(hero.currentHp)?hero.currentHp:hero.stats.maxHp))}));this.guild=new Guild(save.guild);this.zoneLevel=Math.max(1,Math.floor(save.zoneLevel||1));this.story={...defaultStory(),...(save.story||{})};
  // Migration intentionally brings existing combat saves into town without touching progression.
